...
void *MyEthThread (void *arg) {
	...
	targets[0].events = POKKIN | POLLERR;
...
int show_ifreq(char *name){
	...
	if ((soc = socket(AT_INET, SOCK_DGRAM, 0)) == -1){
		perror("socket");
		return (-1);
	}
	...
	if (ioctl(soc, SIOCGIFFLAG, &ifreq ) == -1){
		perror("ioctl:flag");
		close(soc);
		return (-1);
	}
...
#include ""
	...
	if(GetMacAddress(name, Param.myma) == -1){
